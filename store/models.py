...
# ,ResizeToCover,ResizeToFill,SmartResize
...
def has_changed(instance, field):
    ...
    old_value = instance.__class__._default_manager.filter(
        id=instance.pk).values(field).get()[field]
...
class Customer(models.Model):
    ...
    Cart = models.OneToOneField('Cart', on_delete=models.CASCADE,
                                related_name='Customer', blank=True, default=None, null=True)
    ...
    PhoneNumber = models.CharField(
        max_length=10, help_text='10 Digit Phonenumber')
    TotalPurchase = models.PositiveIntegerField(default=0, blank=True)
    NumberOfOrders = models.PositiveSmallIntegerField(
...
class Order(models.Model):
    ...
    Method = models.ForeignKey('PaymentMethod', related_name='Orders',
                               editable=False, on_delete=models.SET_NULL, null=True)
    # Meta Details
    Categories = models.ManyToManyField(
        'Categorey', related_name='In_orders', editable=False, blank=True)
    SubCategories = models.ManyToManyField(
        'SubCategorey', related_name='In_orders', editable=False, blank=True)
    SellerComments = models.TextField(default='No Comments From Seller!')
    DaysForCompletion = models.PositiveSmallIntegerField(
        default=None, null=True, blank=True, editable=False)
    FeedbackRating = models.PositiveSmallIntegerField(
        default=None, null=True, blank=True, editable=False)
    FeedbackComment = models.TextField(
        max_length=600, default=None, null=True, blank=True, editable=False)
...
class OrderProduct(models.Model):
    Product = models.ForeignKey(
        'Product', on_delete=models.PROTECT, related_name='In_orders')
    Order = models.ForeignKey(
        Order, related_name='Orderprod', on_delete=models.CASCADE)
...
class OrderVariant(models.Model):
    Variant = models.ForeignKey(
        'Variant', on_delete=models.PROTECT, related_name='In_orders')
    Order = models.ForeignKey(
        Order, on_delete=models.CASCADE, related_name='Ordervar')
    Quantity = models.PositiveSmallIntegerField()
    Added = models.DateTimeField(auto_now_add=True)
...
class CartProduct(models.Model):
    ...
    Cart = models.ForeignKey(
        'Cart', related_name='Cartproduct', on_delete=models.CASCADE)
...
class CartVariant(models.Model):
    Variant = models.ForeignKey(
        'Variant', on_delete=models.CASCADE, related_name='In_carts')
    Quantity = models.PositiveSmallIntegerField()
    Added = models.DateTimeField(auto_now_add=True)
    Cart = models.ForeignKey(
        'Cart', related_name='Cartvariant', on_delete=models.CASCADE)
    ForLater = models.BooleanField(default=False)
...
class ShippingAddress(models.Model):
    Owner = models.ForeignKey(
        Customer, on_delete=models.CASCADE, related_name='ShippingAddressOptions')
    Father = models.CharField(
        max_length=164, verbose_name='(C/O) Gaurdian Name', blank=True, default='Not Applicable')
    OwnerName = models.CharField(max_length=256, verbose_name='Name')
    PhoneNumber = models.BigIntegerField(verbose_name='Main Phone Number', validators=[MaxValueValidator(
        9999999999), MinValueValidator(1000000000)], help_text='Your Main Phone Number')
    AltPhoneNumber = models.BigIntegerField(verbose_name='Alternate [ Second ] Phone Number', validators=[MaxValueValidator(
        9999999999), MinValueValidator(1000000000)], help_text='If First Phone Number Will Not Work, We Will Use This')
    ...
    LandMark = models.CharField(
        max_length=164, help_text='(Optional)\nEx-opposite SBI or near this building...')
    Address = models.TextField(
        max_length=720, help_text='Area Name and Gali/Street Number')
    AddressType = models.CharField(max_length=164, choices=ADDRESSTYPES, default='HOME',
                                   help_text=f'HOME : For All Day Delivery <br> OFFICE : For Delivery Between 9AM - 6PM')
...
class TopBar(models.Model):
    ...
    Name = models.CharField(max_length=256, verbose_name='Topbar Name',
                            help_text='Example - Newyear Topbar, New Session Topbar')
    Active = models.BooleanField(
        default=False, verbose_name='Show on Main Page')
...
class Topbar_images(models.Model):
    Name = models.CharField(max_length=48, verbose_name='Categorey Name',
                            help_text='Example - Markers, GK Books, Pencil Colors')
    ...
    Image = ProcessedImageField(upload_to='genimages',
                                ...
                                format='JPEG',
                                options={'quality': 80})
    Related_topbar = models.ForeignKey(
        'Topbar', related_name='Images', on_delete=models.CASCADE)
...
class Slider(models.Model):
    ...
    Active = models.BooleanField(
        default=False, verbose_name='Show on Main Page')
    SliderCount_pc = models.PositiveSmallIntegerField(
        editable=False, blank=True, default=0)
    SliderCount_mob = models.PositiveSmallIntegerField(
        editable=False, blank=True, default=0)
...
class Slide_pc(models.Model):
    ...
    Related_slider = models.ForeignKey(
        Slider, related_name='Slide_pc', on_delete=models.CASCADE)
    GotoLink = models.URLField(
        max_length=200, verbose_name='Linked URL/Page', blank=True, default='')


class Slide_mob(models.Model):
    Image = models.ImageField(upload_to='slider/mob_images')
    Related_slider = models.ForeignKey(
        Slider, related_name='Slide_mob', on_delete=models.CASCADE)
    GotoLink = models.URLField(
        max_length=200, verbose_name='Linked URL/Page', blank=True, default='')
...
class Categorey(models.Model):
    ...
    Name = models.CharField(max_length=64, verbose_name='Categorey Group Name',
                            help_text='Example - Schools, Classwise, Office, Pen and Pencils')
    Created = models.DateTimeField(auto_now_add=True)
...
class SubCategorey(models.Model):
    ...
    Parent_categorey = models.ManyToManyField(
        Categorey, verbose_name='In Categorey Group', related_name='Subcategorey')
...
class Product_type(models.Model):
    ...
    Created = models.DateTimeField(auto_now_add=True)
    Last_updated = models.DateTimeField(auto_now=True)
    Meta_data = ArrayField(models.CharField(
        max_length=150), default=return_array, verbose_name='Details')
...
class Product_images(models.Model):
    ...
    Main_image = models.ImageField(
        upload_to='products/images/highres', help_text='This will be the high quality image')
    ...
    Related_product = models.ForeignKey(
        'Product', related_name='Its_images', on_delete=models.CASCADE, default=1)
...
class Product(models.Model):
    ...
    Is_normal_product = models.BooleanField(
        default=True, editable=False, blank=True)
    Created = models.DateTimeField(auto_now_add=True)
    Last_updated = models.DateTimeField(auto_now=True)
    Added_by = models.CharField(
        default='Rohit admin', max_length=64, blank=True, editable=False)
    # Important Details
    Name = models.CharField(max_length=512)
    Brand = models.ForeignKey(
        Brand, related_name='Brans_products', on_delete=models.PROTECT)
    ...
    Highlights = ArrayField(models.CharField(
        max_length=164), default=return_array)
    Type = models.ForeignKey(
        Product_type, related_name='All_products', on_delete=models.PROTECT)
    Meta_data = JSONField(default=return_dict)
    Cost_to_customer = models.PositiveIntegerField(
        help_text='Price that customer will PAY!')
    Cost_to_seller = models.PositiveIntegerField(
        help_text='Total Cost of product for seller')
    Crossed_price = models.PositiveIntegerField(
        help_text='Price that will be crossed')
    Local_offer = models.PositiveIntegerField(validators=[MaxValueValidator(
        90)], blank=True, default=0, help_text='Discount percentage (%)')
    Is_active = models.BooleanField(default=True)
    Total_stock = models.PositiveIntegerField()
    Categorey = models.ManyToManyField(
        SubCategorey, related_name='All_products')
    Tags = models.ManyToManyField(Tag, related_name='All_taged_products')
    # Calculation Details
    Rating = models.FloatField(validators=[MaxValueValidator(
        5), MinValueValidator(0)], editable=False, default=0)
    ...
    Total_carts = models.PositiveIntegerField(
        default=0, blank=True, editable=False)
    Sales_probablity = models.FloatField(
        validators=[MaxValueValidator(1)], default=1, blank=True, editable=False)
    Cart_probablity = models.FloatField(validators=[MaxValueValidator(
        1), MinValueValidator(0)], default=1, blank=True, editable=False)
    ...
    def save(self, *args, **kwargs):
        ...
        self.Discount = round(
            ((self.Crossed_price - self.Cost_to_customer)/self.Crossed_price)*100)
        print(self.Discount)
...
class ProductRow(models.Model):
    ...
    Products = models.ManyToManyField(
        Product, related_name='In_row', blank=True)
    Variants = models.ManyToManyField(
        'Variant', related_name='In_row', blank=True)
    Show_on_mainpage = models.BooleanField(default=False)
...
class Collection(models.Model):
    ...
    Last_updated = models.DateTimeField(auto_now=True)
    Products = models.ManyToManyField(
        Product, related_name='In_collection', blank=True)
    Variants = models.ManyToManyField(
        'Variant', related_name='In_collection', blank=True)
...
class ProductAttributeValue(models.Model):
    ...
    Attribute = models.ForeignKey(
        ProductAttribute, related_name='Attributevalue', on_delete=models.CASCADE)
    Variant = models.ForeignKey(
        'Variant', related_name='Attributevalue', on_delete=models.CASCADE, null=True)
...
class ProductWithVariant(models.Model):
    Is_variant = models.BooleanField(default=False, editable=False, blank=True)
    Is_normal_product = models.BooleanField(
        default=False, editable=False, blank=True)
    Created = models.DateTimeField(auto_now_add=True)
    Last_updated = models.DateTimeField(auto_now=True)
    ...
    Thumb_path = models.CharField(max_length=364, blank=True, editable=False)
    Type = models.ForeignKey(
        Product_type, related_name='All_products_with_variant', on_delete=models.PROTECT)
    Highlights = ArrayField(models.CharField(
        max_length=164), default=return_array)
    Meta_data = JSONField(default=return_dict)
    Categorey = models.ManyToManyField(
        SubCategorey, related_name='All_products_with_variant')
    Tags = models.ManyToManyField(
        Tag, related_name='All_products_with_variant')
    Rating = models.FloatField(validators=[MaxValueValidator(
        5), MinValueValidator(0)], editable=False, default=0)
    Nums_of_rating = models.PositiveIntegerField(
        default=0, blank=True, editable=False)
    Is_active = models.BooleanField(
        default=True, verbose_name='Product Status')
    ...
    def save(self, *args, **kwargs):
        print(
            f'-------[In ProductWithVariant]-----{self.Thumbnail.url}----------')
        self.Thumb_path = self.Thumbnail.url
...
class Variant(models.Model):
    ...
    Highlights = ArrayField(models.CharField(
        max_length=164), default=return_array, blank=True, editable=False)
    ...
    Crossed_price = models.PositiveIntegerField(
        help_text='Price that will be crossed')
    Local_offer = models.PositiveIntegerField(validators=[MaxValueValidator(
        90)], blank=True, default=0, help_text='Discount percentage (%)', verbose_name='Discount in ( % )')
    Is_active = models.BooleanField(
        default=True, verbose_name='Product Status')
    Total_stock = models.PositiveIntegerField()
    Product = models.ForeignKey(
        "ProductWithVariant", on_delete=models.PROTECT, related_name='Variants')
...
class Fileshare(models.Model):
    ...
    ImageFile = models.ImageField(
        upload_to='fileshare', blank=True, null=True, default=None)
    File = models.FileField(upload_to='fileshare/files',
...
class Banner(models.Model):
    ...
    Type = models.CharField(choices=BANNER_TYPES,
                            default='normal', max_length=64)
